...
export function piece3(options: piece3Options): Rule {
    ...
    const newOptions = {
        modPath: `src/app/biz/shared/biz.shared.module.ts`,
    ...
    return chain([
        branchAndMerge(chain([
            ...
            //new module.ts
            addImport(newOptions.modPath, newOptions.symbolName, newOptions.componentPath),
            addValToVar(newOptions.modPath, "COMPONENTS", newOptions.symbolName),

            //edit module.ts
            addImport(editOptions.modPath, editOptions.symbolName, editOptions.componentPath),
            addValToVar(editOptions.modPath, "COMPONENTS", editOptions.symbolName)
        ]))
    ]);
}